...
def get_iv_greeks(date, rf, stock_prc, df_contract, df_dividend):
    ...
    for i in range(len(daily_contract_code)):
        ...
        code_list.append(str(daily_contract_code.at[i, '期权代码']))
        ...
        contract_unit.append(daily_contract_code.at[i, '合约单位'])
    ...
    df_iv = pd.DataFrame({'strike_prc': strike_list, 'code': code_list, 'callput': callput_list, 'maturity': maturity_list,
                          'ttm': ttm_list, 'iv': iv_list, 'delta': delta_list,'contract_unit': contract_unit})
...
def get_daily_iv_delta(code, date, df_rf, df_contract, df_stock):
    """
    计算每天所有期权合约的IV和Delta
    :param code: 标的代码（不带'.SH')
    :param date: 日期
