...
def openshort(df, rank, percent):
    opens = False
    #设>10条件意味着不做short
    if ((test(df,rank,percent) < df.iloc[rank])) & (df.iloc[rank] > 10):
        opens = True
    return opens
...
def switch(i,df_tradedate,code_50,code_25,choice,kind):
    #choice=1:long skew; choice=-1:short skew
    #kind=1, 换50delta期权； kind=2, 换25delta期权； kind=3, 全换
    (price_50,delta_50,vega_50,gamma_50,theta_50) = getinfor(df_tradedate,code_50)
    (price_25,delta_25,vega_25,gamma_25,theta_25) = getinfor(df_tradedate,code_25)
...
window = 20
cost = 2 #期权交易费用
short = 80 #short开仓分位数(忽略)
long = 15 #long开仓分位数
holding = 0 #long skew holding=1，short skew holding=-1
close_s = 50 #short平仓分位数(忽略)
close_l = 45 #long平仓分位数
numk = 500  #买卖系数
...
df_records.to_excel('../结果/call.xlsx')
