...
for ps_open in lst_ps_open:
    for cl_open in lst_cl_open:
        ...
        def switch_flag(etf,k_old,t_old,option_type): #option_type: 'ps'/'cl'
            # Check if need to change the K, put itm switch, call itm switch
        ...
        df_records['daily_r_etf'] = df_records['etf'].pct_change()
        df_records['daily_r_etf'][0] = 0
        
        
        df_records.loc[:,'净值'] = (df_records.loc[:,'PnL%']+1).cumprod()
        
        print("Update df_records %s(收盘后)"%etf_kind)
        df_records.to_excel('./%s/fut_%s_collar_%s_%s_%s_ps_itm_switch.xlsx'%(etf_kind,etf_kind,datetime.datetime.now().strftime('%m%d'),ps_open,cl_open), index = False) 
        
          
        df_records.rename(columns={'PnL%': 'daily_r_%s_%s'%(ps_open,cl_open), '净值': '净值_%s_%s'%(ps_open,cl_open)}, inplace=True)
        if result.empty == True:
            result = df_records.loc[:,['Date','daily_r_%s_%s'%(ps_open,cl_open),'净值_%s_%s'%(ps_open,cl_open)]]
        else:
            result = pd.merge(result,df_records.loc[:,['Date','daily_r_%s_%s'%(ps_open,cl_open),'净值_%s_%s'%(ps_open,cl_open)]],on = 'Date', how = 'outer')
        
result = pd.merge(result,df_records.loc[:,['Date','daily_r_etf','etf_chg']],on = 'Date', how = 'outer')
...
writer = pd.ExcelWriter("./%s_collar_fut_%s_result_combine.xlsx"%(etf_kind,(datetime.datetime.now().strftime('%m%d'))), engine='xlsxwriter')
...
result.filter(regex='Date|净值|etf_chg').to_excel(writer, sheet_name='Performance',index= False)
