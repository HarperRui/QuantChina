...
class collar_etf_300index():
   def __init__(self, etf_kind, change_day=10, cost_t=1.7, cost_i=0.5, cost_IH = 0.000023, cost_etf = 0.00003):
        """
        Parameters
        ----------
   ...
   def cal_position(self,i,yesterday, date, etf_yes, etf, df_tradedate,df_contract, ps_open, cl_open, ps_maintain , cl_maintain): # buy S + buy put, sell call
        #kind: 'call'/'put'
        remark_ps = ''
        remark_cl = ''
        ...
        try:
            ...
            settle_yes = p_cl
        margin = self.margin_single_option(settle_yes,k_cl,num_cl,etf_yes,etf,'call') #short call
       ...
       '''  
        ...
        self.printRecord(i,remark_ps+' '+remark_cl, etf, num_etf,
   ...
   def printRecord(self,i,Remark, index, Num_index,
                    Strike_ps, Code_ps, Num_ps, No_ps, lot_ps, t_ps,
                    Strike_cl, Code_cl, Num_cl, No_cl, lot_cl,t_cl):
        ...
        self.df_records.loc[i,'Num_index'] = Num_index
   ...
   def cal_cost(self,i):
        ...
        if i > 0:
            ...
            if i ==1:
                #IH
                cost = abs(self.df_records.Num_index[i] * self.df_records.index[i]) * self.cost_etf
                ...
                cost = abs(self.df_records.Num_index[i]-self.df_records.Num_index[i-1])* self.df_records.index[i] * self.cost_etf
   ...
   def backtest_loop(self,ps_open, cl_open, ps_maintain , cl_maintain):    
        df_rf, df_options, df_etf, df_contract,IH, datelist, last_date_records = self.df_rf, self.df_options, self.df_etf, self.df_contract,self.IH, self.datelist, self.last_date_records
        ...
        for i in range(1,len(datelist)): 
            ...
            #Calculate position
            self.cal_position(i,yesterday, date, etf_yes, etf_close, df_date,df_contract,ps_open, cl_open, ps_maintain , cl_maintain)
            ...
            self.df_records.loc[i,'Margin'] = self.margin_single_option(settle_yes,self.df_records.Strike_cl[i],self.df_records.Num_cl[i],etf_yes,etf_close,'call')
            ...
            self.df_records.loc[i,'Underlying'] =  etf_close * self.df_records.Num_index[i]
            
            self.df_records.loc[i,'Cost'] = -self.cal_cost(i)
            
            self.df_records.loc[i,'Cash'] = self.df_records.loc[i,'Margin'] + self.df_records.loc[i,'Option'] + self.df_records.loc[i,'Underlying'] + abs(self.df_records.loc[i,'Cost'])
            ...
            if pd.isnull(self.df_records.Code_ps[i-1]) == False:
                ...
                self.df_records.at[i,'PnL_index'] = (etf_close - etf_yes) * self.df_records.Num_index[i-1]
                
                self.df_records.loc[i,'PnL'] = self.df_records['PnL_option'][i] + self.df_records['PnL_index'][i] + self.df_records['Cost'][i]
        ...
        self.df_records.to_excel('./%s/%s_collar_%s_%s_%s.xlsx'%(self.etf_kind,self.etf_kind,datetime.datetime.now().strftime('%m%d'),ps_open,cl_open), index = False) 
...
if __name__=='__main__':
    ...
    writer = pd.ExcelWriter("./%s/%s_collar_%s_result_combine.xlsx"%(index.etf_kind,index.etf_kind,(datetime.datetime.now().strftime('%m%d'))), engine='xlsxwriter')
    ...
    print('finish')
    #Plot
    fig = result.plot(title='300index_etf').get_figure()
    fig.tight_layout()
