...
# Create df_records
df_records = pd.DataFrame(columns = ['Date', 'Signal','Remark', 'etf','Num_etf', 
       'Strike_ps', 'Code_ps', 'Num_ps', 'No_ps', 'Lot_ps','T_ps',
       'Strike_cl', 'Code_cl', 'Num_cl','No_cl', 'Lot_cl','T_cl',
       'Margin', 'Option','Underlying', 'Cost', 'Cash','PnL_option', 'PnL_etf', 'PnL', 'PnL%','净值'])

df_rf, df_options, df_underlying, df_contract,IH = get_database_data.read_database(etf_kind,'close')
...
def printRecord(i,Remark, etf, Num_etf,
                Strike_ps, Code_ps, Num_ps, No_ps, lot_ps, t_ps,
                Strike_cl, Code_cl, Num_cl, No_cl, lot_cl,t_cl):
    ...
    df_records.loc[i,'Num_etf'] = Num_etf
...
def cal_cost(i):
    ...
    if i > 0:
        ...
        if i ==1:
            #IH
            cost = abs(df_records.Num_etf[i] * df_records.etf[i]) * cost_etf
            ...
            cost = abs(df_records.Num_etf[i]-df_records.Num_etf[i-1])* df_records.etf[i] * cost_etf
...
for i in range(1,len(datelist)): 
    ...
    df_records.loc[i,'Underlying'] =  etf_close * df_records.Num_etf[i]
    ...
    if pd.isnull(df_records.Code_ps[i-1]) == False:
        ...
        df_records.at[i,'PnL_etf'] = (etf_close - etf_yes) * df_records.Num_etf[i-1]
        
        df_records.loc[i,'PnL'] = df_records['PnL_option'][i] + df_records['PnL_etf'][i] + df_records['Cost'][i]
...
df_records.to_excel('./%s_collar_%s_%s_%s_chgday8.xlsx'%(etf_kind,datetime.datetime.now().strftime('%m%d'),ps_open,cl_open), index = False) 
